...
display_summary() {
    ...
    if [ "$ENABLE_INSTALL_WORKER" = "true" ]; then
        for ((i = 0; i < ${#KUBESLICE_WORKERS[@]}; i++)); do
            ...
            worker_name=$(yq e ".kubeslice_worker_egs[$i].name" "$KUBETALLY_INPUT_YAML")
            skip_installation=$(yq e ".kubeslice_worker_egs[$i].skip_installation" "$KUBETALLY_INPUT_YAML")
            kubeconfig=$(yq e ".kubeslice_worker_egs[$i].kubeconfig" "$KUBETALLY_INPUT_YAML")
            kubecontext=$(yq e ".kubeslice_worker_egs[$i].kubecontext" "$KUBETALLY_INPUT_YAML")
            ...
            namespace=$(yq e ".kubeslice_worker_egs[$i].namespace" "$KUBETALLY_INPUT_YAML")
            release_name=$(yq e ".kubeslice_worker_egs[$i].release" "$KUBETALLY_INPUT_YAML")
            chart_name=$(yq e ".kubeslice_worker_egs[$i].chart" "$KUBETALLY_INPUT_YAML")
    ...
    if [ "$ENABLE_INSTALL_ADDITIONAL_APPS" = "true" ]; then
        for ((i = 0; i < ${#ADDITIONAL_APPS[@]}; i++)); do
            ...
            chart_name=$(yq e ".additional_apps[$i].chart" "$KUBETALLY_INPUT_YAML")
...
if [ "$ENABLE_INSTALL_WORKER" = "true" ]; then
    for worker_index in "${!KUBESLICE_WORKERS[@]}"; do
        ...
        worker=$(yq e ".kubeslice_worker_egs[$worker_index]" "$KUBETALLY_INPUT_YAML")
